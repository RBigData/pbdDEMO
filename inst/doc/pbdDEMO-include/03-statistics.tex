...
\item
What are assumptions for the Strong Law of Large Numbers?
...
prove $\bar{X}_N$ converges to $\frac{\pi}{4}$ in probability and
in distribution.

\item
As Statement~(\ref{eqn:as_convergence}),
Prove $4\bar{X}_N$ converges to $\pi$ almost surely, in probability and
in distribution.
...
\item
Prove $\bX^\top\bX$ is non-negative definite if $\bX$ is full rank $p$,
