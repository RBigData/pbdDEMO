...
The ``Programming with Big Data in R'' project~\citep{pbdR2012} (pbdR
...
Figure~\ref{fig:pbdrpackages} shows the current list of pbdR packages released
to the CRAN (\url{http://cran.r-project.org}), and how they depend on each
other. More explicitly, the current \proglang{pbdR} packages are:

\begin{itemize}
 \item \pkg{pbdMPI} --- an efficient interface to MPI~\citep{MPI1994} with a
       focus on Single Program/Multiple Data (SPMD)\index{SPMD} parallel
       programming style.
 \item \pkg{pbdSLAP} --- bundles scalable dense linear algebra libraries in
       double precision for \proglang{R}, based on ScaLAPACK
       version 2.0.2~\citep{slug}.~\index{ScaLAPACK}
 \item \pkg{pbdNCDF4} --- Interface to Parallel Unidata NetCDF4\index{NetCDF4}
       format data files~\citep{netcdf}.
...
\proglang{pbdR} packages into those meant for users, developers, or
...
users of \proglang{pbdR} packages, while \pkg{pbdBASE} and \pkg{pbdSLAP} are
...
\begin{figure}[h]
 ...
 \label{fig:pbdrlibs}
\end{figure}
Figure~\ref{fig:pbdrlibs} shows \proglang{pbdR} relationship to
high-performance libraries.

In this vignette, we offer many examples using the above \proglang{pbdR}
...
pre-existing functions and utilities of \proglang{pbdR} to create minor
...
the various \proglang{pbdR} packages. The second is this vignette, which
...
Enter \proglang{pbdR}. Through our project, we put a shiny new set of
...
\end{lstlisting}
Since \pkg{pbdEMO} depends on other \proglang{pbdR} packages, please read
...
cores, the computation is performed faster using \proglang{pbdR}
...
\item
Read \pkg{pbdMPI}'s vignettes and install either
OpenMPI~\index{OpenMPI} (\url{http://www.open-mpi.org/}) or
MPICH2~\index{MPICH2} (\url{http://www.mcs.anl.gov/research/projects/mpich2/}),
 and test if installation is correct.
\label{ex:introduction1}

\item
When finish Exercise~\ref{ex:introduction1},
install all \proglang{pbdR} packages and run demo codes.
\end{enumerate}
