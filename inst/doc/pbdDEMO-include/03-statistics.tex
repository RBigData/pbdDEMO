...
Another fundamental tool in the statistician's toolbox is finding quantiles.  Quantiles are points taken from the cumulative distribution function.  Formally, a $q$-quantile (or ``$q$-tile'') with $q \in [0, 1]$ of a random variable $X$ is any value $\theta_q$ such that
\begin{align*}
P(X\leq \theta_q) = q
\end{align*}
In the case of a continuous random variable, this value exists and is unique.  Often, we define the $k$'th $q$-quantile, which we take as above, with the modification that $P(X\leq \theta_q) = kq$, where we think of sub-dividing the interval $[0,1]$ into $q-1$ almost-evenly spaced subsets.  The likely the reason for this more general definition is that arguably the most common use of quantiles is in constructing Q-Q plots, where this scheme is necessary.  Additionally, some authors instead take $q\in(0, \infty)$, so that, for example, in the above notation the $0.5$-quantile (the median) becomes the ``2-quantile.''

This example can be extended to construct Q-Q plots, compute cumulative density function estimates and nonparametric statistics, as well as solve maximum likelihood estimators.  This is perhaps an inefficient implementation to approximate a quantile and is not equivalent to the original \code{quantile()} function in
...
is often numerically non-stable.  Instead, it is generally better (although much slower) to take an orthogonal factorization of the data matrix.  Typically, the QR-decomposition is used to this end.  Here $\bX=QR$, where $Q$ is orthogonal and $R$ is upper trapezoidal.  This is beneficial, because orthogonal matrices are norm-preserving, i.e. $Q$ is an isometry, and whence
\begin{align*}
\ltwo{\bX\bbeta - \by} &= \ltwo{QR\bbeta - \by}\\
  &= \ltwo{Q^TQR\bbeta - Q^T\by}\\
  &= \ltwo{R\bbeta - Q^T\by}
\end{align*}
The (arguably) much more well-known Singular Value Decomposition can also be used to develop yet another algebraically identical solution which is quite elegant.  Here, if we take the \emph{reduced} (or \emph{compact}) SVD $\bX = U\Sigma V^T$ where here we only take the first $rank(\bX)$ singular values, then it can be shown that ``the'' desired solution is given by
\begin{align*}
\ltwo{\bX\bbeta - \by} &= \ltwo{U\Sigma V^T\bbeta - \by}\\
  &= \ltwo{\Sigma V^T\bbeta - U^T\by}
\end{align*}
