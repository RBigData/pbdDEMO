...
\item {\bf\color{blue} Q:}
      Do I need MPI knowledge to run \pkg{pbdDEMO}? \\
      {\bf\color{blue} A:}
      Yes, but only the big picture, not the details. We provide
      several examples in \code{pbdDEMO/inst/examples/test_spmd/} to
      introduce essential methods for learning MPI communication.

\item {\bf\color{blue} Q:}
      Can I run \pkg{pbdDEMO} on my laptop locally? \\
      {\bf\color{blue} A:}
      Sure, as long as you have an MPI system. You even can run it on 1 CPU.

\item {\bf\color{blue} Q:}
      Does \pkg{pbdDEMO} support Windows clusters? \\
      {\bf\color{blue} A:}
      Yes, the released binary currently supports OpenMPI and MPICH2.
      For other MPI systems, users have to compile from source.

\item {\bf\color{blue} Q:}
      Can I run \pkg{pbdDEMO} in OpenMPI and MPICH2 together? \\
      {\bf\color{blue} A:}
      No, you can have both OpenMPI and MPICH2 installed in your OS, but
      you are only allowed to run \pkg{pbdDEMO} with one MPI system.
      Just pick one.

\item {\bf\color{blue} Q:}
      Does \pkg{pbdDEMO} support any interactive mode? \\
      {\bf\color{blue} A:}
      No, but yes. \pkg{pbdDEMO} only considers batch execution and
      ...
      you can utilize \pkg{pbdDEMO} scripts inside
      ...
      Note that \pkg{pbdDEMO} uses communicators different from
      \pkg{Rmpi}.  Be sure to free the memory correctly for both
      packages before quitting. \code{finalize(mpi.finalize = FALSE)}
      can free the memory allocated by \pkg{pbdDEMO}, but does not
...
\item {\bf\color{blue} Q:}
      ...
      Quick examples can be found in \code{pbdDEMO/inst/examples/test_s4/}.
\end{enumerate}
...
\begin{enumerate}
\item {\bf\color{blue} Q:}
      Can I run task jobs by using \pkg{pbdDEMO}? \\
      {\bf\color{blue} A:}
...
\item {\bf\color{blue} Q:}
      What if I want to run task push or pull by using \pkg{pbdDEMO}? \\
      {\bf\color{blue} A:}
...
%\item {\bf\color{blue} Q:}
%      Can I disable the MPI initialization of \pkg{pbdDEMO} when I call
%      \code{library(pbdDEMO)}? \\
%      {\bf\color{blue} A:}
%      Yes, you can set a hidden variable \code{.__DISABLE_MPI_INIT__} in the
%      \code{.GlobalEnv} before calling \code{library(pbdDEMO)}.
%      For example,
%\begin{Code}[title=SPMD R Script]
...
%      by \pkg{pbdDEMO} has to be free, \code{mpi.finalize = FALSE} is set above.
%
%      To avoid some initialization issues of MPI, \pkg{pbdDEMO} uses a
%      different way than \pkg{Rmpi}. \pkg{pbdDEMO} allows you to disable
...
\item {\bf\color{blue} Q:}
      Can \pkg{pbdDEMO} take or export communicators? \\
      {\bf\color{blue} A:}
      ...
      structure arrays preallocated while \pkg{pbdDEMO} is loaded.
      \code{pbdDEMO/src/pkg_*} provides a mechanism to take or export
...
Error in dyn.load(file, DLLpath = DLLpath, ...) :
  unable to load shared object '/.../pbdDEMO/libs/pbdDEMO.so':
...
\item {\bf\color{blue} Q:}
      \pkg{pbdDEMO} installs successfuly, but fails at initialization when
...
%}
      ...
      file \code{libmpi.so} is used by \code{pbdDEMO/src/spmd.c}
      through \code{\#include <dlfcn.h>} and \code{dlopen(...)} in the file
      \code{pbdDEMO/src/pkg_dl.c}.
      ...
      \item When linking with \code{libmpi.so} in OpenMPI, one must run/load
            \pkg{pbdDEMO} with OpenMPI's \code{libmpi.so}. The same for
            LAM/MPI and MPICH2.
