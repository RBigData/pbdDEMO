...
The NetCDF4 is built on top of HDF5 data model for extremely large and complex
...
The \proglang{R} package \pkg{ncdf4}~\citep{ncdf4}
...
\begin{table}[t]
\caption{\pkg{pbdNCDF4} and \pkg{ncdf4} functions for accessing NetCDF4
files}
\label{tab:ncdf4}
\centering
...
The \pkg{pbdDEMO} has an example dataset \code{TREFHT} from a
...
through the C20C project (on the NERSC jportal).
\begin{figure}[t]
  \centering
  \includegraphics[width=6.0in]{pbdDEMO-include/pics/trefht}
  \caption{Monthly averaged temperature at reference height (TREFHT) in
