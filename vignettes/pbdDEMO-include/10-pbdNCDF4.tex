...
Note that \code{demo.ncvar_put_dmat()} and \code{demo.ncvar_get_dmat()} are
implemented for 2D variables only. Please use \pkg{pbdNCDF4}/\pkg{ncdf4}
primitive functions
...
\emph{Example: Dump and read 9 by 9 (1D and 2D) ddmatrices in paralle NetCDF4.}
...
These examples create a subdirectory \code{./nc4_data} first,
then generate 81 1D NetCDF4 parallel files \code{./nc4_data/1d_*_*.nc},
and 81 2D NetCDF4 parallel files \code{./nc4_data/2d_*_*.nc},
and then read them back in. Both reading and writing are in \code{ddmatrix}
and in parallel NetCDF4 formats. Note that the storing formate of nc files
are in row major, but \proglang{R} use column major to store a matrix.
However, these examples will not have conflicts since they define rows
as the frist variable, so transpose matrices will be actually seen
from \code{ncdump}. As long as read and write are in the same order,
everything is fine. Further, the \code{ddmatrix} is actually convert
to \code{gbdr} or \code{gbdc} formats, then dump to or read from the
NetCDF4 files in parallel. This could be inefficient for a large matrix,
so use with caution!
...
\item
Implement functions or add arguments to the put method,
\code{demo.ncvar_put_dmat()}, and the get method,
\code{demo.ncvar_get_dmat()},
