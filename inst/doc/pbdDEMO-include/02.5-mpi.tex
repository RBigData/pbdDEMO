...
\item Modify the script in Exercise~\ref{ex:mpi1} above to determine if any processors sampled \code{TRUE}.  Do the same to determine if all processors sampled \code{TRUE}.  In each case, print the result.  Compare to the functions \code{comm.all()} and \code{comm.any()}.  (Hint:  use \code{allreduce()}).
