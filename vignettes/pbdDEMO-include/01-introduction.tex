...
\mylabel{
\begin{rotate}{270}
\hspace{1cm}\hyperref[sec:introduction]{Chapter 1}
\end{rotate}
\vspace{3.7cm}\hrule
\begin{rotate}{270}
\hspace{1cm}\hyperref[sec:bg]{Chapter 2}
\end{rotate}
\vspace{3.7cm}\hrule
}
...
% % 
% % 
% % \hyperref[sec:bg]{Chapter 2}\\[.2cm]
% % \hrule\vspace{.2cm}
% % \hyperref[chp:mpi_for_the_r_user]{Chapter 3}\\[.2cm]





\inspire{There is nothing new under the sun. It has all been done before.}{Sherlock Holmes}


\section{What is pbdR?}

The ``Programming with Big Data in R'' project~\citep{pbdR2012}
(\proglang{pbd} or \proglang{pbdR}
...
Figure~\ref{fig:pbdrpackages} shows the current list of \proglang{pbdR}
packages released
to the CRAN (\url{http://cran.r-project.org}), and how they depend on each
other. More explicitly, the current \proglang{pbdR} packages
...
preexisting functions and utilities of \proglang{pbdR} to create minor
...
install all \proglang{pbdR} packages and run each package's demo codes.
\end{enumerate}
