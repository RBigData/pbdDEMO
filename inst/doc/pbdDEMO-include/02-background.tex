...
A very loose pseudocode for this procedure might look like:
...
\code{ddmatrix} format,~\index{code!\code{ddmatrix}} although we provide many tools to ease the pains of doing so.  Learning how to use these packages can greatly improve code performance, and take your statistical modeling in \proglang{R} to previously unimaginable scales.

Again for the sake of understanding, we will at times append the suffix
\code{.dmat}~\index{code!\code{.dmat}} to objects of class \code{ddmatrix} to differentiate them from the more general \code{.spmd} object.  As with \code{.spmd}, this carries no semantic meaning, and is merely used to improve the readability of example code (especially when managing both ``\code{.spmd}'' and \code{ddmatrix} objects).
...
\item
\proglang{R} users are focusing on task parallelism and master/workers
...
\code{fork} mechanism.
Compare this with OpenMP~\citep{OpenMP}\index{OpenMP}.
