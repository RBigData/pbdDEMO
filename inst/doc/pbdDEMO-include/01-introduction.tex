...
functions of \pkg{pbdDEMO}~\citep{Chen2012}, version 0.1-1.
...
\pkg{pbdDEMO} is intended for batch mode programming with big data
...
this direction.  \pkg{pbdDEMO} simplifies MPI interaction, but leaves
low and mid level functions available for advanced programmers. For
example, it is easy to hand communicators to \pkg{pbdDEMO} from other
...
% \pkg{pbdDEMO} provides a better way for communications
...
In the following, we list the main features of \pkg{pbdDEMO}.
\begin{enumerate}
...
\item Basic data types of integer, double and raw in \pkg{pbdDEMO} are
...
System requirements and installation of \pkg{pbdDEMO} are described
next.  Section~\ref{sec:performance} gives a short example for
comparing performance of \pkg{pbdDEMO} and \pkg{Rmpi}~\citep{Yu2010}.
...
two examples from \pkg{parallel} are shown as SPMD \pkg{pbdDEMO}
programs.
...
to unix, \pkg{pbdDEMO} should also run under other operating systems
...
Note that the static library is not necessary for \pkg{pbdDEMO} but may
...
\end{Command}
See the package source file \code{pbdDEMO/configure} for details.

One can get started quickly with \pkg{pbdDEMO} by learning from the
following six examples.
\begin{Command}
### At the shell prompt, run the demo with 2 processors by
### (Use Rscript.exe for windows system)
mpiexec -np 2 Rscript -e "demo(allgather,'pbdDEMO',ask=F,echo=F)"
mpiexec -np 2 Rscript -e "demo(allreduce,'pbdDEMO',ask=F,echo=F)"
mpiexec -np 2 Rscript -e "demo(bcast,'pbdDEMO',ask=F,echo=F)"
mpiexec -np 2 Rscript -e "demo(gather,'pbdDEMO',ask=F,echo=F)"
mpiexec -np 2 Rscript -e "demo(reduce,'pbdDEMO',ask = F,echo=F)"
mpiexec -np 2 Rscript -e "demo(scatter,'pbdDEMO',ask=F,echo=F)"
\end{Command}
...
shows typical basic steps of using \pkg{pbdDEMO}.
\begin{enumerate}
...
code. \pkg{pbdDEMO} tends to reduce programming effort, avoid
complicated MPI techniques, and gain computing performance.

The major communication functions of \pkg{pbdDEMO} and corresponding
similar functions of \pkg{Rmpi} are listed in the following.
\\
\begin{center}
\vspace{0.2cm}
\begin{tabular}{cl} \hline\hline
\pkg{pbdDEMO} (S4) & \pkg{Rmpi}                \\ \hline
...
The package source files provide several examples based on \pkg{pbdDEMO},
such as \\
\begin{center}
\vspace{0.2cm}
\begin{tabular}{ll} \hline\hline
Directory & Examples \\ \hline
\code{pbdDEMO/inst/examples/test_spmd/}         & main SPMD functions \\
\code{pbdDEMO/inst/examples/test_rmpi/}         & comparison to \pkg{Rmpi} \\
\code{pbdDEMO/inst/examples/test_parallel/}     & comparison to \pkg{parallel} \\
\code{pbdDEMO/inst/examples/test_performance/}  & performance testing \\
\code{pbdDEMO/inst/examples/test_s4/}           & S4 extension \\
\hline\hline
\end{tabular}
\end{center}
